...
# get ATR of card and display it on error output
sys.stderr.write( "ATR: " + toHexString( cardservice.connection.getATR() ) + "\n")

## This function decrypts the chunk key using the SmartCard:
def decryptKey( key ):
  global cardservice
  ## Form the command to be sent to the card:
  DECRYPT_KEY = [0x88, 0x10, 0, 0, len( list(key) )] + list( key ) + [0x10]
...
def processChunk( chunk ):
  ...
  sys.stderr.write( "New key: " + toHexString( list( newKey ) ) + "\n")
  decryptEngine = Crypto.Cipher.AES.new( newKey, Crypto.Cipher.AES.MODE_ECB )
  decryptData = b""
  decryptData = decryptEngine.decrypt( encData )
  sys.stdout.buffer.write( decryptData )
#end def
...
indata = sys.stdin.buffer.read( chunksize * n )
position = indata.rfind( preamble.encode() )
remaining = position - chunksize * n + chunksize
if remaining > chunksize:
  ## Preamble is exactly at end of buffer:
  position = position + chunksize
  remaining = remaining - chunksize
indata = indata[position:] + sys.stdin.buffer.read( remaining )

while 1:
  processChunk( indata )
  indata = sys.stdin.buffer.read( chunksize )
  if not indata:
